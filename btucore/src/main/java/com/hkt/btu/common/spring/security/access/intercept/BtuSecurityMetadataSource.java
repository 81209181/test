...
public class BtuSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    // unused
    //public static final String RESERVED_ANT_PATH_OAUTH2_LOGIN = "/oauth*/**";
    // unused
    //public static final String RESERVED_ANT_PATH_OAUTH2_LOGIN2 = "/login/oauth*/**";
    // using, for keycloak
    public static final String RESERVED_ANT_PATH_OAUTH2_LOGIN3 = "/sso/login";
    ...
    public Map<RequestMatcher, Collection<ConfigAttribute>> buildResourceMapFromDb() {
        ...
        //newResourceMap.put(new AntPathRequestMatcher(RESERVED_ANT_PATH_OAUTH2_LOGIN), CONFIG_LIST_PERMIT_ALL);
        //newResourceMap.put(new AntPathRequestMatcher(RESERVED_ANT_PATH_OAUTH2_LOGIN2), CONFIG_LIST_PERMIT_ALL);
        newResourceMap.put(new AntPathRequestMatcher(RESERVED_ANT_PATH_OAUTH2_LOGIN3), CONFIG_LIST_PERMIT_ALL);
    ...
    public boolean isEligibleUrl(String url, Collection<GrantedAuthority> authorities) {
        List<ConfigAttribute> configs = new ArrayList<>();
        for (Map.Entry<RequestMatcher, Collection<ConfigAttribute>> entry : getResourceMap().entrySet()) {
            RequestMatcher key = entry.getKey();
            if (key instanceof AntPathRequestMatcher) {
                AntPathRequestMatcher antKey = (AntPathRequestMatcher) key;
                String pattern = antKey.getPattern();
        ...
        for (ConfigAttribute configAttribute : configs) {
            if (BtuSecurityMetadataSource.CONFIG_ATTR_DENY_ALL.equals(configAttribute)) {
                continue;
            } else if (BtuSecurityMetadataSource.CONFIG_ATTR_PERMIT_ALL.equals(configAttribute)) {
                return true; // pass
            }
