...
public class BtuEmailServiceImpl implements BtuEmailService {
    ...
    public void send(String templateId, String recipient, Map<String, Object> dataMap) throws MessagingException {
        ...
        injectGlobalEmailData(dataMap, siteConfigBean);
        ...
        if (!siteConfigService.isProductionServer()) {
            subjectString = String.format("[%s] %s", siteConfigBean.getServerType(), subjectString);
        }
    ...
    private void injectGlobalEmailData(Map<String, Object> dataMap, BtuSiteConfigBean siteConfigBean) {
        if (dataMap == null) {
            dataMap = new HashMap<>();
        }
    ...
    private InternetAddress buildSenderProfile(BtuSiteConfigBean siteConfigBean) {
        try {
            String fromAddress = siteConfigBean.getMailUsername();
            String fromName = siteConfigBean.getAppName();
