...
function diffColor(diffs,blockLeftId,blockRightId){for(i=0;i<diffs.length;i++){let linesLeft=$(`#${blockLeftId}`).text().split('\n');let textLeft=$(`#${blockLeftId}`).text();for(let j=0;j<linesLeft.length;j++){if(diffs[i].path1.line==j+1&&diffs[i].type=='eq'||diffs[i].type=='type'){$(`#${blockLeftId}`).text(textLeft.replace(linesLeft[j],`<span style="background-color: yellow;">${linesLeft[j]}</span>`))}else if(diffs[i].path1.line==j+1&&diffs[i].type=='right'){$(`#${blockLeftId}`).text(textLeft.replace(linesLeft[j],`<span style="color:#c0392b;">${linesLeft[j]}</span>`))}}
let linesRight=$(`#${blockRightId}`).text().split('\n');let textRight=$(`#${blockRightId}`).text();for(let j=0;j<linesRight.length;j++){if(diffs[i].path2.line==j+1&&diffs[i].type=='eq'){$(`#${blockRightId}`).text(textRight.replace(linesRight[j],`<span style="background-color: yellow;">${linesRight[j]}</span>`))}
